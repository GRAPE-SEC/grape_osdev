...
int main(int argc, char *argv[])
{
        ...
        if (argv < 3){
                fprintf(stderr, "[ERROR] Usage: %s <BootLoader.bin path> <Kernel32.bin path>\n", argv[0]);
                exit(-1);
        }

        if ((iTargetFd = open("Disk.img", O_RDWR | O_CREAT | O_TRUNC | O_BINARY, S_IREAD | S_IWRITE)) == -1){
                fprintf(stderr, "[ERROR] Creation failed: Disk.img\n");
                exit(-1);
        }
...
// copy iSourceFD contents to the iTargetFD and return size of the copied data
int CopyFile(int iSourceFd, int iTargetFd)
{
        int iSourceFileSize;
        int iRead;
